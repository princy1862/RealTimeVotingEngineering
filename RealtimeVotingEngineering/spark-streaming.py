...
if __name__ == '__main__':
    # print(pyspark.__version__) = 4.0.0
    # initializing spark session
    # Ensure Spark is using the correct Java version
    os.environ['JAVA_HOME'] = '/opt/homebrew/opt/openjdk@17'

    spark= (SparkSession.builder
            .appName("RealTimeVotingEngineering" )
            .config('spark.jars.packages','org.apache.spark:spark-sql-kafka-0-10_2.13:4.0.0') #spark-kafka integration
            .config('spark.jars','/Users/princypatel/PycharmProjects/RealtimeVotingEngineering/postgresql-42.7.7.jar') #postgresql driver
...
#    Aggregate votes per candidate and turnout by location
    ...
    # Kafka output streams
    votes_per_candidate_to_kafka = (votes_per_candidate.selectExpr("to_json(struct(*)) AS value")
                                    .writeStream
                                    .format("kafka")
                                    .option("kafka.bootstrap.servers", "localhost:9092")
                                    .option("topic", "aggregated_votes_per_candidate")
                                    .option("checkpointLocation","/Users/princypatel/PycharmProjects/RealtimeVotingEngineering/checkpoints/checkpoint1")
                                    .outputMode("update")
                                    .start()
                                    )
    turnout_by_location_to_kafka = (turnout_by_location.selectExpr("to_json(struct(*)) AS value")
                                    ...
                                    .option("checkpointLocation",
                                            "/Users/princypatel/PycharmProjects/RealtimeVotingEngineering/checkpoints/checkpoint2")
                                    .outputMode("update")
                                    .start()
                                    )
