...
def fetch_voting_stats():
    ...
    cur.execute("""
        SELECT count(*) voters_count FROM votes
    """)
...
def update_data():
    ...
    if data:
        ...
        # Check if results DataFrame has the required columns
        if not results.empty and 'candidate_id' in results.columns and 'total_votes' in results.columns:
            # Identify the leading candidate
            results = results.loc[results.groupby('candidate_id')['total_votes'].idxmax()]
            leading_candidate = results.loc[results['total_votes'].idxmax()]
        else:
            st.warning("⚠️ Waiting for voting data from Kafka... Please ensure Spark Streaming is running.")
            return
    else:
        st.info("📊 No real-time data available yet. Starting up the data pipeline...")
        return
